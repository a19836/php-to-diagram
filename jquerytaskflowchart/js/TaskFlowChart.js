 ...
 *
 * Original JQuery Task Flow Chart Repo: https://github.com/a19836/jquery-task-flow-chart/
 * Original Bloxtor Repo: https://github.com/a19836/bloxtor
 *
 * YOU ARE NOT AUTHORIZED TO MODIFY OR REMOVE ANY PART OF THIS NOTICE!
 */

//CREATING THE taskFlowChartObj object from the TaskFlowChart class.
;(function() {
	window.taskFlowChartObj = new TaskFlowChart("taskFlowChartObj");
})();
...
//DEFINING TaskFlowChart FUNCTION
function TaskFlowChart(taskFlowChartObjVarName, taskFlowChartObjOptions) {
	 ...
	 * }
	 */
	WF.taskFlowChartObjOptions = taskFlowChartObjOptions;
	
	WF.setTaskFlowChartObjOption = function(option_name, option_value) {
		if (!WF.taskFlowChartObjOptions || !$.isPlainObject(WF.taskFlowChartObjOptions))
			WF.taskFlowChartObjOptions = {};
		
		WF.taskFlowChartObjOptions[option_name] = option_value;
	};
	...
	WF.getExternalLibCloneBridge = function() {
		return ExternalLibCloneBridge;
	};
	
	WF.getTaskFlowChartObjVarName = function() {
		return taskFlowChartObjVarName;
	};
	...
	WF.init = function() {
		WF.onReady(function() {
			...
			ExternalLibClone = ExternalLibCloneBridge.instance;
			...
			if (WF.taskFlowChartObjOptions && WF.taskFlowChartObjOptions.hasOwnProperty("on_init_function") && typeof WF.taskFlowChartObjOptions.on_init_function == "function")
				WF.taskFlowChartObjOptions.on_init_function(WF);
	...
	WF.initOptions = function() {
		if (WF.taskFlowChartObjOptions) {
			var parent = $("#" + WF.TaskFlow.main_tasks_flow_obj_id).parent().closest(".taskflowchart");
			
			if (WF.taskFlowChartObjOptions["resizable_task_properties"])
				parent.addClass("resizable_task_properties");
			else if (parent.hasClass("resizable_task_properties"))
				WF.taskFlowChartObjOptions["resizable_task_properties"] = true;
			
			if (WF.taskFlowChartObjOptions["resizable_connection_properties"])
				parent.addClass("resizable_connection_properties");
			else if (parent.hasClass("resizable_connection_properties"))
				WF.taskFlowChartObjOptions["resizable_connection_properties"] = true;
			
			if (WF.taskFlowChartObjOptions["fixed_properties"])
				parent.addClass("fixed_properties");
			else if (parent.hasClass("fixed_properties"))
				WF.taskFlowChartObjOptions["fixed_properties"] = true;
			
			if (WF.taskFlowChartObjOptions["fixed_side_properties"])
				parent.addClass("fixed_side_properties");
			else if (parent.hasClass("fixed_side_properties"))
				WF.taskFlowChartObjOptions["fixed_side_properties"] = true;
			
			if (WF.taskFlowChartObjOptions["on_init_function"] && typeof WF.taskFlowChartObjOptions["on_init_function"] == "string")
				eval('WF.taskFlowChartObjOptions["on_init_function"] = ' + WF.taskFlowChartObjOptions["on_init_function"] + ';');
			
			if (WF.taskFlowChartObjOptions["on_resize_panels_function"] && typeof WF.taskFlowChartObjOptions["on_resize_panels_function"] == "string")
				eval('WF.taskFlowChartObjOptions["on_resize_panels_function"] = ' + WF.taskFlowChartObjOptions["on_resize_panels_function"] + ';');
			
			if (WF.taskFlowChartObjOptions["on_toggle_properties_panel_side_function"] && typeof WF.taskFlowChartObjOptions["on_toggle_properties_panel_side_function"] == "string")
				eval('WF.taskFlowChartObjOptions["on_toggle_properties_panel_side_function"] = ' + WF.taskFlowChartObjOptions["on_toggle_properties_panel_side_function"] + ';');
		}
	};
	
	WF.resizePanels = function() {
		var workflow_menu = $("#" + WF.ContextMenu.main_workflow_menu_obj_id);
		var tasks_menu = $("#" + WF.ContextMenu.main_tasks_menu_obj_id);
		var tasks_menu_hide = $("#" + WF.ContextMenu.main_tasks_menu_hide_obj_id);
		...
		WF.TaskFlow.zoomRefresh();
		
		if (WF.taskFlowChartObjOptions && WF.taskFlowChartObjOptions.hasOwnProperty("on_resize_panels_function") && typeof WF.taskFlowChartObjOptions.on_resize_panels_function == "function")
			WF.taskFlowChartObjOptions.on_resize_panels_function(WF, height);
	};
	...
	WF.reinit = function() {
		...
		for (var i = 0; i < tasks.length; i++) {
			...
			if (task.id)
				WF.TaskFlow.deleteTask(task.id, {confirm: false});
		};
	...
	WF.changeRenderMode = function(desiredMode) {
		if (WF.TaskFile.reset)
			WF.TaskFile.reset();
			
		if (WF.Property.reset)
			WF.Property.reset();
		
		if (WF.Container.reset)
			WF.Container.reset();
		
		if (WF.ContextMenu.reset)
			WF.ContextMenu.reset();
		
		if (WF.TaskFlow.reset) 
			WF.TaskFlow.reset();
	...
	WF.destroy = function() {
		//clean first ExternalLibClone and other elements in workflow
		WF.reinit(); //very important otherwise if we call the repaint or any other method the ExternalLibClone will try to execute that action in an element that doesn't exist anymore, returning an exception...
		
		if (typeof ExternalLibClone.destroy == "function")
			ExternalLibClone.destroy();
		
		//destroy context menu
		WF.ContextMenu.destroy();
		
		var selector = "#" + WF.TaskFlow.main_tasks_flow_obj_id + 
					", #" + WF.TaskFlow.main_tasks_properties_obj_id + 
					", #" + WF.TaskFlow.main_connections_properties_obj_id + 

					", #" + WF.Property.selected_task_properties_id + 
					", #" + WF.Property.selected_connection_properties_id + 

					", #" + WF.ContextMenu.main_tasks_menu_obj_id + 
					", #" + WF.ContextMenu.main_tasks_menu_hide_obj_id + 
					", #" + WF.ContextMenu.task_context_menu_id + 
					", #" + WF.ContextMenu.connection_context_menu_id + 

					", #" + WF.TaskFile.update_task_panel_id + 
					", #" + WF.TaskFile.update_connection_panel_id + 

					", #" + WF.StatusMessage.message_html_obj_id;
	...
	WF.TaskFlow = { 
		...
		default_connection_line_width : 1,
		default_connection_from_target : false,
		default_connection_z_index : 10,
		default_similar_connections_gap : 20,
		...
		init : function() {
			...
			main_tasks_flow_obj.addClass("taskflowchartdiagram");
			...
			var defaults_to_import = {
				...
				Endpoint : ["Dot", {radius:4}], //only work in jsPlumb
				ConnectionOverlays : this.getDefaultConnectionOverlays(),
				ConnectorZIndex: this.default_connection_z_index,
				
				//for leader-line
				startPointSVG: '<svg viewBox="-5 -5 10 10" width="8" height="8"><use href="#leader-line-disc" x="0" y="0" /></svg>',
				//startPointSVG: '<svg viewBox="0 0 8 8" width="8" height="8"><use href="#leader-line-square" x="4" y="4" /></svg>',
				endPointSVG: '<svg viewBox="-5 -5 10 10" width="8" height="8"><use href="#leader-line-disc" x="0" y="0" /></svg>',
				//endPointSVG: '<svg viewBox="0 0 8 8" width="8" height="8"><use href="#leader-line-square" x="4" y="4" /></svg>',
				on_connection_body_class: "task_flow_chart_dragging_body",
				dragging_hover_target_class: "dragHover",
				dragging_active_target_class: "dragActive",
				//overlay_class: "connector_overlay",
				//end_point_class: "end_point",
				similar_lines_gap: this.default_similar_connections_gap,
			};
			...
			ExternalLibClone.bind("contextmenu", function(connection, originalEvent) {
				...
				var is_target = connection.hasOwnProperty("isSource") && !connection.isSource && connection.hasOwnProperty("isTarget") && connection.getElement && connection.getElement().hasClass(WF.TaskFlow.task_class_name) && connection.getElement().attr("id");
				
				//PREPARE TASK CONTEXT MENU
				if(is_source || is_target) {
					var target_id = is_source ? connection.getElement().attr("target_id") : connection.getElement().attr("id");
					var target_elm = WF.TaskFlow.getTaskById(target_id);
					var task_type = target_elm.attr("type");
			  		
			  		if (task_type) {
				  		var show_context_menu = WF.Property.isTaskSubSettingTrue(task_type, "task_menu", "show_context_menu", true);
				  		
				  		if (show_context_menu)
					  		WF.ContextMenu.showTaskMenu(originalEvent, target_elm[0]);
				  	}
				  
				  	//FIXING A SMALL BUG FROM JSPLUMB
				  	if (ExternalLibCloneBridge.isJsPlumb()) {
						var c = WF.TaskFlow.getConnection(connection.id);
						
						if (!c && connection.canvas && $(connection.canvas).is(".end_point")) //only for jsplumb
				...
				//PREPARE CONNECTION CONTEXT MENU
					...
					if (task_type) {
						var show_context_menu = WF.Property.isTaskSubSettingTrue(task_type, "connection_menu", "show_context_menu", true);
				  		
				  		if (show_context_menu)
							WF.ContextMenu.showConnectionMenu(originalEvent, connection);
					}
				}
		
				//WF.TaskFlow.printConnection("contextmenu", connection);
				...
				if (task_type) {
					...
					var target = originalEvent && originalEvent.target ? $(originalEvent.target) : null;
			  		...
			  		if (show_context_menu && is_arrow_target) {
		  				...
		  				WF.Property.callTaskSettingsCallback(task_type, "on_click_connection", [ connection ]);
		  			}
				}
			...
			ExternalLibClone.bind("dblclick", function(connection, originalEvent) {
				...
				var new_type = WF.TaskFlow.getNextConnectionConnectorType(connection_exit_type);
				WF.TaskFlow.changeConnectionConnectorType(connection, new_type);
				
				//WF.TaskFlow.printConnection("dblclick", conn);
				
				return false;
			});
		  	
		  	ExternalLibClone.bind("connection", function(conn) {
				//console.log(conn);
				...
				var source_task_id = conn.sourceId;
			  	...
			  	var connector_canvas = $(conn.connection.canvas);
			  	var parameters = conn.connection.getParameters();
				//console.log(parameters);
				var source_task_tag = WF.TaskFlow.getTaskById(source_task_id).attr("tag");
		  		var target_task_tag = WF.TaskFlow.getTaskById(target_task_id).attr("tag");
				...
				if ( (exists_connection_exit_id = parameters.hasOwnProperty("connection_exit_id")) ) {
					...
					connection_label = null; //Do not set label bc label will be set later in the WF.TaskFlow.connect method
				...
				if (!exists_connection_exit_id || !connection_exit_id) {
					...
					WF.StatusMessage.showError("ERROR: There was a problem connecting this 2 tasks. Please refresh the page and if the problem persists, please contact the SysAdmin.");
					return false;
				}
				
				//sets the correct connection_type
				if (connection_type)
					WF.TaskFlow.changeConnectionConnectorType(conn.connection, connection_type);
				
				//sets the connection_overlay
				if (connection_overlay)
					WF.TaskFlow.changeConnectionOverlayType(conn.connection, connection_overlay);
				
				//sets the correct connection_color
				if (color)
					WF.TaskFlow.changeConnectionColor(conn.connection, color);
				...
				//set some classes
				connector_canvas.addClass("connector"); //add connector class. Use classList.add instead of addClass, bc the addClass doesn't work on SVG nodes.
				
				$(conn.sourceEndpoint.canvas).addClass("end_point source_end_point end_point_for_source_task_" + source_task_tag + " end_point_for_target_task_" + target_task_tag);
				$(conn.targetEndpoint.canvas).addClass("end_point target_end_point end_point_for_source_task_" + source_task_tag + " end_point_for_target_task_" + target_task_tag);
				
				var overlays = conn.connection.getOverlays();
				
				if (overlays)
					for (var i = 0; i < overlays.length; i++) 
						if (overlays[i].canvas) {
							var overlay_canvas = $(overlays[i].canvas);
							
							overlay_canvas.addClass("connector_overlay connector_overlay_for_source_task_" + source_task_tag + " connector_overlay_for_target_task_" + target_task_tag);
							overlay_canvas.attr("connection_id", conn.connection.id);
							overlay_canvas.hover(
								function(e) {
									overlay_canvas.parent().addClass("connector_over");
								},
								function(e) {
									overlay_canvas.parent().removeClass("connector_over");
								}
							);
						}
				
				//set over handler
				var connector_path = connector_canvas.find(".leader-line-path");
				
				connector_path.hover(
					function(e) {
						connector_canvas.parent().children(".leader-line-overlay-" + conn.connection.id).addClass("connector_over");
					},
					function(e) {
						connector_canvas.parent().children(".leader-line-overlay-" + conn.connection.id).removeClass("connector_over");
					}
				);
					...
					setTimeout(function() {
						if (typeof window.getSelection !== "undefined") {
							...
							if (!focus_elm || focus_elm != label_overlay_elm[0])
								WF.TaskFlow.setConnectionLabelByOverlay(label_overlay);
						}
					}, 500);
					
					//return WF.TaskFlow.setConnectionLabelByOverlay(label_overlay);
				});
				label_overlay_elm.on("blur", function(originalEvent) {
					WF.TaskFlow.saveConnectionLabelByOverlay(label_overlay, {
				...
				//label_overlay.setLabel( WF.TaskFlow.default_label );
				...
				label_overlay.getElement().setAttribute("connection_id", conn.connection.id);
			  	...
			  	if (!loaded_from_file) {
				  	if (WF.Property.tasks_settings) {
				  		var status = true;
				  		
				  		//check if connection is allowed. Example if parent task is a "function" type, doesn't allow outside connections.
				  		if (!WF.TaskFlow.isConnectionAllowed(source_task_id, target_task_id)) {
				  			WF.StatusMessage.showError("ERROR: You cannot connect these 2 tasks because outside connections are not allowed!");
				  			status = false;
				  		}
				  		else {
					  		var source_task_type = WF.TaskFlow.getTaskById(source_task_id).attr("type");
					  		var target_task_type = WF.TaskFlow.getTaskById(target_task_id).attr("type");
					  		
					  		//call callbacks
					  		if(status && !WF.Property.callTaskSettingsCallback(source_task_type, "on_success_connection_drag", [ conn ]))
					  			status = false;
							
							if(status && !WF.Property.callTaskSettingsCallback(target_task_type, "on_success_connection_drop", [ conn ]))
					  			status = false;
						}
		  		...
		  		//WF.TaskFlow.printConnection("connection", conn);
		  	});
			...
			/*ExternalLibClone.bind("connectionDragStop", function(conn) {
				...
				//	targetEndpoint:undefined
			  	...
			  	//WF.TaskFlow.printConnection("connectionDragStop", conn);
			  	//console.log(conn);
			});*/
		...
		initTargetAndSourceElements : function(options) {
			...
			$(target_selector).each(function(i, target_elm) {
				WF.TaskFlow.addTargetElement(target_elm, connection_exit_props);
				
				$(target_elm).find(source_selector).each(function(j, source_elm) {
					WF.TaskFlow.addSourceElement(source_elm, target_elm, connection_exit_props);
				});
			});
		},
		...
		initTargetElements : function(options) {
			...
			$(target_selector).each(function(i, target_elm) {
				...
				WF.TaskFlow.centerTaskInnerElements(target_elm.id);
			});
		},
		...
		initSourceElements : function(options) {
			...
			$(source_selector).each(function(i, source_elm) {
				$(source_elm).parents(WF.TaskFlow.target_selector).last().each(function(j, parent_elm){
					WF.TaskFlow.addSourceElement(source_elm, parent_elm, connection_exit_props);
				});
			});
		},
		/* END: INIT METHODS */
		...
		addTargetElement : function(target_elm, connection_exit_props) {
			...
			WF.TaskFlow.getTaskLabelElement(j_target_elm).each(function(i, elm) {
				var j_elm = $(elm);
				
				j_elm.attr("contenteditable", "true").attr("spellcheck", "false");
				
				j_elm.off("click").attr('onclick', '').click( function (originalEvent) { 
					originalEvent.stopPropagation(); 
					
					if (originalEvent.preventDefault) originalEvent.preventDefault(); 
					else originalEvent.returnValue = false;
					
					var j_task = j_elm.parents(WF.TaskFlow.target_selector).first();
					var task_id = j_task.attr("id");
					var task_type = j_task.attr("type");
					
					if (WF.Property.callTaskSettingsCallback(task_type, "on_start_task_label", [ task_id ])) {
						j_elm.data("previous_label", j_elm.text());
						j_elm.focus();
						...
						setTimeout(function() {
							if (typeof window.getSelection !== "undefined") {
								...
								if (!focus_elm || focus_elm != elm)
									return WF.TaskFlow.setTaskLabel(elm);
							}
						}, 500);
					}
					else
					...
					var label_obj = {label: j_elm.text()};
					var func = WF.Property.getTaskSubSetting(task_type, "callback", "on_check_task_label");
					var invalid = false;
					
					if (func)
						invalid = WF.Property.callTaskSettingsCallback(task_type, "on_check_task_label", [ label_obj, task_id ]) ? false : true;
			  		
			  		if (!invalid) {
						if (label_obj.label != null) //on_check_task_label may change the label_obj.label to null
					  		WF.TaskFlow.saveTaskLabel(elm, label_obj);
						else 
							WF.Property.callTaskSettingsCallback(task_type, "on_cancel_task_label", [ task_id ]);
					}
					else { //set previous label
						j_elm.text( j_elm.data("previous_label") );
					}
				});
			});
		  	
		  	//set contextmenu
		  	if (WF.ContextMenu.isContextMenuActive()) {
		  		var task_type = target_elm.getAttribute("type");
		  		
		  		var show_context_menu = WF.Property.isTaskSubSettingTrue(task_type, "task_menu", "show_context_menu", true);
		  		
		  		if (show_context_menu) {
			  		j_target_elm.addcontextmenu(WF.ContextMenu.task_context_menu_id, {
			  			callback: function(target, contextmenu, originalEvent) {
							...
							if (main_tasks_flow_obj.data("inner_task_dragging"))
			  				...
			  				WF.ContextMenu.prepareTaskMenu(originalEvent, target_elm);
			  			...
			  			if (main_tasks_flow_obj.data("contextmenu_cannot_be_shown") != 1) {
							...
							originalEvent.pageY = o.top;
				  			...
				  			var ul = $("#" + WF.ContextMenu.task_context_menu_id);
				  			...
				  			//show task menu
				  			WF.ContextMenu.showTaskMenu(originalEvent, target_elm);
				  			
				  			//add handler to be executed everytime a task gets clicked. Example: show its properties
				  			WF.Property.callTaskSettingsCallback(task_type, "on_click_task", [ target_elm.id, j_target_elm ]);
				  		}
			  		});
			...
			var is_js_plumb = ExternalLibCloneBridge.isJsPlumb();
			...
			ExternalLibClone.draggable(ExternalLibClone.getSelector(target_elm), {
				...
				start: function(e, obj) {
					...
					var inner_tasks = j_elm.find("." + WF.TaskFlow.task_class_name);
					...
					var parent_task = WF.TaskFlow.getTaskParentTasks(j_elm).first();
					j_elm.data("parent_task", null);
					
					if (parent_task.is("." + WF.TaskFlow.task_class_name))
				...
				drag: function(e, obj) {
					...
					if (!j_elm.data("parent_task")) {
						//prepare some margins and offset if jsplumb
						if (is_js_plumb) {
							if (!is_firefox) { //only if not firefox
								...
								var current_zoom = WF.TaskFlow.getCurrentZoom();
				...
				stop: function(e, obj) {
					...
					var start_scroll_left = 0;
					
					if (is_js_plumb) {
						start_scroll_top = parseInt(j_elm.css("margin-top"));
						start_scroll_left = parseInt(j_elm.css("margin-left"));
					}
					...
					var current_zoom = WF.TaskFlow.getCurrentZoom();
					var offset_zoom = is_js_plumb ? current_zoom : 1;
					obj.position.left = parseInt(obj.position.left * offset_zoom); 
					obj.position.top = parseInt(obj.position.top * offset_zoom);
					
					//prepare offset with offset_zoom
					var task_offset = assignObjectRecursively({}, obj.offset);
					task_offset.top = parseInt(task_offset.top * offset_zoom);
					task_offset.left = parseInt(task_offset.left * offset_zoom);
					
					//prepare droppable based in zoom bc the '.droppable(' jquery function does not work properly with zoom
					if (current_zoom != 1 && parent_task && !j_elm.data("droppable")) {
						var display = elm.style.display;
						j_elm.hide();
						
						var over_elm = document.elementFromPoint(e.clientX, e.clientY);
						...
						if ($(over_elm).is("." + WF.TaskFlow.task_droppable_class_name))
							j_elm.data("droppable", over_elm);
					}
					
					var status = WF.Property.callTaskSettingsCallback(task_type, "on_task_drag_stop_validation", [ j_elm ]);
					...
					if (!status || (!parent_task && !WF.ContextMenu.checkIfTaskCanBeCreated( task_type, elm, j_elm.offset() ))) {
						WF.StatusMessage.showError("You cannot drop this Task on that position! Please try again...");
						...
						if (droppable) {
							...
							if (droppable.hasClass("connector_overlay_add_icon")) {
								...
								droppable_connection = WF.TaskFlow.getOverlayConnectionId(add_icon);
							...
							if (!parent.is(droppable[0])) {
								...
								if (droppable.is(main_tasks_flow_obj)) {
									var offset = WF.TaskFlow.getNewElmPosition(main_tasks_flow_obj, parent, obj.position);
								...
								//if droppable is a task or inside of a task
									...
									WF.TaskFlow.resizeTaskParentTask(droppable);
						...
						if (droppable_connection)
							WF.ContextMenu.setTaskBetweenConnection(j_elm, droppable_connection);
						
						//repaint all connections for inner tasks if exists
						if (inner_tasks && inner_tasks.length > 0) {
							WF.TaskFlow.resizeTaskBasedOnInnerTasks(j_elm);
						...
						if (parent_task)
							WF.TaskFlow.resizeTaskParentTask(j_elm); //resize parent task with the new width and height according with this inner task
					}
					
					main_tasks_flow_obj.data("inner_task_dragging", null); //reset dragging
					main_tasks_flow_obj.removeClass("dragging_task");
					
					WF.Property.callTaskSettingsCallback(task_type, "on_task_drag_stop_end", [ j_elm ]);
					
					if (current_zoom != 1 && is_js_plumb)
						WF.TaskFlow.repaintEverything(); //very important otherwise the connections will be messy
			...
			var target_options = {
		  	...
		  	if (WF.TaskFlow.default_connection_hover_color)
		  		target_options["hoverPaintStyle"] = { 
					//strokeStyle: WF.TaskFlow.default_connection_hover_color,
					fillStyle: WF.TaskFlow.default_connection_hover_color,
				};
		...
		addSourceElement : function(source_elm, parent_elm, connection_exit_props) {
			...
			var source_options = {
				...
				anchor: "Continuous",
				//anchor: "RightCenter",
				connect_from_target: this.default_connection_from_target,
				connectorStyle: { strokeStyle: on_source_connection_color, lineWidth: this.default_connection_line_width },
				connectorOverlays: overlays,
				container: $("#" + WF.TaskFlow.main_tasks_flow_obj_id), //This container is very important be specified to the main_tasks_flow_obj, bc if there are inner droppable elements, all the sources and targets end-points must be appended to the main_tasks_flow_obj. Otherwise the connection SVG will appear messy and with a wrong position.
				...
				/*dragOptions: {
					start: function(e, obj) {
						var current_zoom = ExternalLibCloneBridge.isJsPlumb() ? WF.TaskFlow.getCurrentZoom() : 1;
		...
		addExtraSourceElementToTask : function(task_id, connection_exit_props) {
			...
			if (task) {
				...
				if (eps) {
					...
					var exit_html = WF.ContextMenu.task_exit_html;
		...
		connect : function(source_task_id, target_task_id, connection_label, connector_type, connection_overlay, connection_exit_props) {
			if (!connection_exit_props || !connection_exit_props.hasOwnProperty("id") || !connection_exit_props.id || !connection_exit_props.color) {
				WF.StatusMessage.showError("ERROR in connecting tasks. Connector exit ID or COLOR is undefined!");
			...
			//Note: Do not need to call the this.setNewConnectionConnector(c) or this.changeConnectionConnectorType(c, connector_type), bc the connection will be updated with the correspondent connector_type in the connection handler
		...
		centerTaskInnerElements : function(task_id) {
			...
			if (task) {
				...
				var status = WF.Property.isTaskSettingTrue(task_type, "center_inner_elements", true);
		...
		repaintTask : function(j_task, position) {
			try {
				if (ExternalLibCloneBridge.getRepaintStatus())
					ExternalLibCloneBridge.repaint(j_task, $.isPlainObject(position) ? position : null);
			}
			catch(e) {
				//console.log(e);
			}
		...
		repaintAllTasks : function() {
			if (ExternalLibCloneBridge.getRepaintStatus())
				$("#" + this.main_tasks_flow_obj_id + " ." + this.task_class_name).each(function(idx, elm){
					WF.TaskFlow.repaintTask( $(elm) );
				});
		},
		
		repaintConnectionByConnectionId : function(connection_id) {
			if (ExternalLibCloneBridge.getRepaintStatus())
				this.repaintConnection( this.getConnection(connection_id) );
		},
		
		repaintConnection : function(connection, options) {
			try {
				if (connection && ExternalLibCloneBridge.getRepaintStatus())
					connection.repaint(options);
			}
			catch(e) {
				//console.log(e);
			}
		...
		repaintEverything : function() {
			try {
				if (ExternalLibCloneBridge.getRepaintStatus())
					ExternalLibCloneBridge.repaintEverything();
			}
			catch(e) {
				//console.log(e);
			}
		...
		deleteConnection : function(connection_id, do_not_confirm) {
			...
			if (connection) {
				...
				if (do_not_confirm || confirm("Do you really want to delete the connection between the tasks: " + source_label + " and " + target_label + "?")) {
					...
					if (WF.Property.isSelectedConnectionPropertiesWithConnectionId(connection_id)) {
						...
						if (parent.is(".fixed_side_properties") && selected_connection_properties.is(".tasks_menu_hidden"))
							WF.ContextMenu.toggleTasksMenuPanel();
					...
					var status = WF.Property.callTaskSettingsCallback(source_task_type, "on_success_connection_deletion", [ connection ]);
					
					if (target_task_type != source_task_type && !WF.Property.callTaskSettingsCallback(target_task_type, "on_success_connection_deletion", [ connection ])) {
						status = false;
					}
					
					WF.TaskFlow.connections_properties[connection_id] = null;
		...
		flipConnection : function(connection_id) {
			...
			if (connection) {
				...
				if (c) {
					...
					var connection_properties = WF.TaskFlow.connections_properties[connection_id];
					WF.TaskFlow.connections_properties[c.id] = connection_properties;
					
					//delete old connection
					WF.TaskFlow.connections_properties[connection_id] = null;
					delete WF.TaskFlow.connections_properties[connection_id];
		...
		//check if connection is allowed. Example if parent task is a "function" type, doesn't allow outside connections.
	  	isConnectionAllowed : function(source_task_id, target_task_id) {
			...
			var source_parent_tasks = WF.TaskFlow.getTaskParentTasks(source_task);
			var source_parent_not_allowed = null;
  			
  			for (var i = 0; i < source_parent_tasks.length; i++) {
  				var source_parent_task = source_parent_tasks[i];
  				
  				if (!WF.Property.isTaskSettingTrue(source_parent_task.getAttribute("type"), "allow_inner_tasks_outside_connections", true)) {
			...
			var target_parent_tasks = WF.TaskFlow.getTaskParentTasks(target_task);
  			var target_parent_not_allowed = null;
  				
  			for (var i = 0; i < target_parent_tasks.length; i++) {
  				var target_parent_task = target_parent_tasks[i];
  				
  				if (!WF.Property.isTaskSettingTrue(target_parent_task.getAttribute("type"), "allow_inner_tasks_outside_connections", true)) {
		...
		saveConnectionLabelByOverlay : function(label_overlay, label_obj) {
			...
			WF.Property.callTaskSettingsCallback(task_type, "on_submit_connection_label", [ label_obj, connection_id ]) ? false : true;
			
			label_obj.label = label_obj.label == "" ? this.default_label : label_obj.label;
			label_overlay.setLabel(label_obj.label);
		
			WF.Property.callTaskSettingsCallback(task_type, "on_complete_connection_label", [ label_overlay, connection_id ]);
		},
		...
		setConnectionLabelByOverlay : function(label_overlay) {
			...
			if (label_obj.label != null)
				...
				WF.Property.callTaskSettingsCallback(task_type, "on_cancel_connection_label", [ connection_id ]);
		...
		prepareConnectionOverlays : function(connection, overlay_type) {
			...
			if (WF.taskFlowChartObjOptions && WF.taskFlowChartObjOptions.hasOwnProperty("is_droppable_connection") && WF.taskFlowChartObjOptions.is_droppable_connection) {
				...
				if (add_icon_overlay) {
					...
					if (!canvas.hasClass("ui-droppable")) {
						var opts = {
							accept: "." + WF.TaskFlow.task_class_name,
							greedy: true,
							tolerance: "pointer",
							drop: function(e, obj) {
		...
		setNewConnectionConnector : function(connection) {
			...
			if (new_connector) {
				...
				if (connection.canvas && connection.canvas.setAttribute)
			  	...
			  	WF.TaskFlow.repaintConnection(connection, {force: true});
		...
		deleteTask : function(task_id, options) {
			if (task_id) {
				...
				if (!to_confirm || (user_confirmation = confirm("Do you really want to delete the task: " + task_label + "?"))) {
					var j_task = this.getTaskById(task_id);
					var task_type = j_task.attr("type");
					var status = WF.Property.callTaskSettingsCallback(task_type, "on_check_task_deletion", [ task_id, j_task ]);
					
					if (status) {
						//hide task properties popup if opened
						if (WF.Property.isSelectedTaskPropertiesWithTaskId(task_id)) {
						...
						if (j_task[0]) {
							...
							//call callback
							var status = WF.Property.callTaskSettingsCallback(task_type, "on_success_task_deletion", [ task_id, j_task ]);
		...
		resizeTaskParentTask : function(task, force) {
			...
			$.each(parents, function(idx, parent) {
				...
				if (!is_resizable_task || force) //only if not resizable
					WF.TaskFlow.resizeTaskBasedOnInnerTasks(parent, force);
			});
		},
				...
				if (droppables.length > 0) {
					...
					$.each(child_droppables, function (idx, child_droppable) {
						...
						$.each(child_droppable_parents, function(idy, parent) {
							var o = WF.TaskFlow.getElementOffsetWithMargin( $(parent) );
							dt += o.top;
							dl += o.left;
						});
						...
						var inner_tasks = child_droppable.children("." + WF.TaskFlow.task_class_name);
						...
						$.each(inner_tasks, function(idx, child) {
							...
							var o = WF.TaskFlow.getElementOffsetWithMargin(child);
		...
		getNewLabel : function(label_text, parent_id, task_type, check_callback_name) {//parent_id == task_id or connection_id
			var label_obj = {label : label_text, from_prompt: true};
			var invalid = false;
			var func = WF.Property.getTaskSubSetting(task_type, "callback", check_callback_name);
			...
			do {
				...
				if (label_obj.label != null) {
					if (func)
						invalid = WF.Property.callTaskSettingsCallback(task_type, check_callback_name, [ label_obj, parent_id ]) ? false : true;
					else
						invalid = false;
				}
		  	} 
		...
		saveTaskLabel : function(elm, label_obj) {
			...
			WF.Property.callTaskSettingsCallback(task_type, "on_submit_task_label", [ label_obj, task_id ]);
			...
			WF.Property.callTaskSettingsCallback(task_type, "on_complete_task_label", [ task_id ]);
		},
		...
		zoom : function(level) {
			...
			//resize containers
			WF.Container.automaticZoomContainers();
	...
	WF.Property = { 
		...
		/* START: INIT METHODS */
		init : function() {
			this.selected_task_properties_id += "_" + Math.abs(WF.TaskFlow.main_tasks_flow_obj_id.hashCode());
			this.selected_connection_properties_id += "_" + Math.abs(WF.TaskFlow.main_tasks_flow_obj_id.hashCode());
		...
		initConnectionProperties : function() {
			var resizable_connection_properties = WF.taskFlowChartObjOptions && WF.taskFlowChartObjOptions.hasOwnProperty("resizable_connection_properties") && WF.taskFlowChartObjOptions.resizable_connection_properties;
			
			var html = '<div id="' + this.selected_connection_properties_id + '" class="myfancypopup ' + this.selected_connection_properties_classes + '">' + 
					'<span class="resize_properties_panel"><div class="button" onClick="' + taskFlowChartObjVarName + '.Property.collapseSelectedConnectionProperties(this)" title="Collapse/Expand Panel"></div></span>' + 
					'<span class="maximize_minimize_icon" onClick="' + taskFlowChartObjVarName + '.Property.maximizeSelectedConnectionProperties(this)" title="Toggle Size"></span>' + 
					'<span class="toggle_properties_side_icon" onClick="' + taskFlowChartObjVarName + '.Property.toggleSelectedConnectionPropertiesSide(this)" title="Toggle Dock Side"></span>' + 
					...
					'<div class="generic_buttons">' + 
						'<input type="button" class="cancel" value="Cancel" onClick="' + taskFlowChartObjVarName + '.Property.cancelConnectionProperties()" />' + 
				 		'<input type="button" class="save" value="Save" onClick="' + taskFlowChartObjVarName + '.Property.saveConnectionProperties()" />' + 
					'</div>' + 
				 '</div>';
			...
			if (resizable_connection_properties)
				WF.Property.prepareResizablePropertiesPanel(selected_connection_properties);
			
			WF.Property.hideSelectedConnectionProperties();
		},
	
		initTaskProperties : function() {
			var resizable_task_properties = WF.taskFlowChartObjOptions && WF.taskFlowChartObjOptions.hasOwnProperty("resizable_task_properties") && WF.taskFlowChartObjOptions.resizable_task_properties;
			
			var html ='<div id="' + this.selected_task_properties_id + '" class="myfancypopup ' + this.selected_task_properties_classes + '">' + 
					'<span class="resize_properties_panel"><div class="button" onClick="' + taskFlowChartObjVarName + '.Property.collapseSelectedTaskProperties(this)" title="Collapse/Expand Panel"></div></span>' + 
					'<span class="maximize_minimize_icon" onClick="' + taskFlowChartObjVarName + '.Property.maximizeSelectedTaskProperties(this)" title="Toggle Size"></span>' + 
					'<span class="toggle_properties_side_icon" onClick="' + taskFlowChartObjVarName + '.Property.toggleSelectedTaskPropertiesSide(this)" title="Toggle Dock Side"></span>' + 
					...
					'<div class="content" stopPropagation="1"></div>' + 
				 	'<div class="generic_buttons">' + 
						'<input type="button" class="cancel" value="Cancel" onClick="' + taskFlowChartObjVarName + '.Property.cancelTaskProperties()" />' + 
				 		'<input type="button" class="save" value="Save" onClick="' + taskFlowChartObjVarName + '.Property.saveTaskProperties()" />' + 
					'</div>' + 
				 '</div>';
			...
			if (resizable_task_properties)
				WF.Property.prepareResizablePropertiesPanel(selected_task_properties);
		...
		prepareResizablePropertiesPanel : function(properties_panel) {
			...
			properties_panel.draggable({
				...
				drag: function(event, ui) {
					...
					WF.Property.resizePropertiesPanel(properties_panel, h);
				},
		...
		resizePropertiesPanel : function(properties_panel, height) {
			...
			var panels = is_fixed_side_properties ? $("#" + WF.ContextMenu.main_tasks_menu_obj_id) : (
							is_fixed_properties ? $("#" + WF.TaskFlow.main_tasks_flow_obj_id + ", #" + WF.ContextMenu.main_tasks_menu_obj_id + ", #" + WF.ContextMenu.main_tasks_menu_hide_obj_id) : null
						);
			...
			if (diff >= wh - 30) { //30 is the size of properties_panel .resize_properties_panel when collapsed
				...
				if (properties_panel.hasClass("selected_connection_properties"))
					WF.Property.addSelectedConnectionPropertiesCollapseClass(properties_panel);
				else if (properties_panel.hasClass("selected_task_properties"))
					WF.Property.addSelectedTaskPropertiesCollapseClass(properties_panel);
				...
				if (properties_panel.hasClass("collapsed_properties")) {
					...
					if (properties_panel.hasClass("selected_connection_properties"))
						WF.Property.removeSelectedConnectionPropertiesCollapseClass(properties_panel);
					else if (properties_panel.hasClass("selected_task_properties"))
						WF.Property.removeSelectedTaskPropertiesCollapseClass(properties_panel);
				}
		...
		togglePropertiesPanelSide : function(properties_panel, properties_panel_resizable) {
			...
			if (parent.hasClass("fixed_side_properties")) { //it will show the fixed_properties
				...
				if (properties_panel_resizable) {
					//add draggable event
					WF.Property.prepareResizablePropertiesPanel(properties_panel);
				}
				
				//resize fixed_side_properties panels according with tasks_menu_hide
				WF.Property.updateFixedSidePropertiesPanelWidth();
			}
			
			//remove hard code style from tasks_flow, tasks_menu and tasks_menu_hide
			var panels = $("#" + WF.TaskFlow.main_tasks_flow_obj_id + ", #" + WF.ContextMenu.main_tasks_menu_obj_id + ", #" + WF.ContextMenu.main_tasks_menu_hide_obj_id);
			...
			if (properties_panel_resizable) {
				WF.Property.resizePropertiesPanel(properties_panel, properties_panel.outerHeight());
			...
			//callback
			if (WF.taskFlowChartObjOptions && WF.taskFlowChartObjOptions.hasOwnProperty("on_toggle_properties_panel_side_function") && typeof WF.taskFlowChartObjOptions.on_toggle_properties_panel_side_function == "function")
				WF.taskFlowChartObjOptions.on_toggle_properties_panel_side_function(properties_panel, properties_panel_resizable);
		},
		...
		updateFixedSidePropertiesPanelWidth : function() {
			//resize fixed_side_properties panels according with tasks_menu_hide
			var selected_task_properties = $("#" + WF.Property.selected_task_properties_id);
			var selected_connection_properties = $("#" + WF.Property.selected_connection_properties_id);
			var tasks_menu_hide = $("#" + WF.ContextMenu.main_tasks_menu_hide_obj_id);
			var parent = tasks_menu_hide.parent().closest(".taskflowchart");
		...
		showSelectedConnectionProperties : function(options) {
			...
			var connection_id = WF.ContextMenu.getContextMenuConnectionId();
			...
			if (connection) {
				...
				if (parent.is(".fixed_side_properties") && selected_connection_properties.is(".tasks_menu_hidden")) {
					if (is_connection_id_different)
						WF.ContextMenu.toggleTasksMenuPanel(); //unhide connection properties
					else
						return false;
				}
				...
				if (is_selected_connection_properties_visible && selected_connection_properties.attr("connection_id") && is_connection_id_different) {
					...
					if (WF.Property.auto_save)
						status = WF.Property.saveConnectionProperties(options);
					else 
						status = WF.Property.cancelConnectionProperties();
				...
				var label = connection.getOverlay("label").getLabel();
				label = label == "" || label == "..." ? WF.TaskFlow.default_label : label;
			
				var style = connection.getPaintStyle();
				var color = style.strokeStyle;
			
				var source_label = WF.TaskFlow.getTaskLabelByTaskId(source_id);
				var target_label = WF.TaskFlow.getTaskLabelByTaskId(target_id);
				...
				*/
				...
				properties += $("#" + WF.TaskFlow.main_connections_properties_obj_id + " .connection_properties_" +  source_type.toLowerCase()).html();
				...
				//toggle some classes
				var panels = $("#" + WF.TaskFlow.main_tasks_flow_obj_id + ", #" + WF.ContextMenu.main_workflow_menu_obj_id + ", #" + WF.ContextMenu.main_tasks_menu_obj_id + ", #" + WF.ContextMenu.main_tasks_menu_hide_obj_id);
					...
					MyFancyPopupClone.init({
						...
						onClose : function() {
							if (!WF.Property.from_hide_selected_connection_properties_func) {
								...
								if (WF.Property.auto_save)
									status = WF.Property.saveConnectionProperties();
								else 
									status = WF.Property.cancelConnectionProperties();
								
								WF.Property.from_hide_selected_connection_properties_func = false;
								
								if (!status) {
									panels.removeClass("connection_properties_shown");
									
									WF.Property.removeCollapseSelectedConnectionProperties(expand_collapse_icon[0]);
								}
							}
							
							if (resizable_connection_properties) {
								WF.Property.resizePropertiesPanel(selected_connection_properties, 0);
								WF.Property.removeSelectedConnectionPropertiesCollapseClass(selected_connection_properties); //the resizePropertiesPanel add the collapsed_properties class
							}
						}
					});
		...
		toggleSelectedConnectionPropertiesSide : function(btn) {
			...
			WF.Property.togglePropertiesPanelSide(selected_connection_properties, resizable_connection_properties);
			WF.Property.removeCollapseSelectedConnectionProperties(btn);
		...
		collapseSelectedConnectionProperties : function(btn) {
			...
			if (resizable_connection_properties) {
				if (selected_connection_properties.hasClass("collapsed_properties"))
					WF.Property.resizePropertiesPanel(selected_connection_properties, 30); //30 is the height of the selected_connection_properties when collapsed
				else
					setTimeout(function() { //must be with settimeout with at least .5 sec, bc the selected_connection_properties has the css transition:.5s all;
						WF.Property.resizePropertiesPanel(selected_connection_properties, selected_connection_properties.outerHeight());
					}, 600);
		...
		showTaskProperties : function(task_id, options) {
			if (task_id) {
				var main_tasks_properties_obj = $("#" + WF.TaskFlow.main_tasks_properties_obj_id);
				...
				if (task_type) {
					...
					if (parent.is(".fixed_side_properties") && selected_task_properties.is(".tasks_menu_hidden")) {
						if (is_task_id_different)
							WF.ContextMenu.toggleTasksMenuPanel(); //unhide task properties
						else
							return false;
					}
					...
					if (is_selected_task_properties_visible && selected_task_properties.attr("task_id") && is_task_id_different) {
						...
						if (WF.Property.auto_save)
							status = WF.Property.saveTaskProperties(options);
						else 
							status = WF.Property.cancelTaskProperties();
						...
						MyFancyPopupClone.init({
							...
							onClose : function() {
								if (!WF.Property.from_hide_selected_task_properties_func) {
									...
									if (WF.Property.auto_save)
										status = WF.Property.saveTaskProperties();
									else 
										status = WF.Property.cancelTaskProperties();
									
									WF.Property.from_hide_selected_task_properties_func = false;
									
									if (!status) {
										panels.removeClass("task_properties_shown");
										
										WF.Property.removeCollapseSelectedTaskProperties(expand_collapse_icon[0]);
									}
								}
								
								if (resizable_task_properties) {
									WF.Property.resizePropertiesPanel(selected_task_properties, 0);
									WF.Property.removeSelectedTaskPropertiesCollapseClass(selected_task_properties); //the resizePropertiesPanel add the collapsed_properties class
								}
							}
						});
		...
		toggleSelectedTaskPropertiesSide : function(btn) {
			...
			WF.Property.togglePropertiesPanelSide(selected_task_properties, resizable_task_properties);
			WF.Property.removeCollapseSelectedTaskProperties(btn);
		...
		collapseSelectedTaskProperties : function(btn) {
			...
			if (resizable_task_properties) {
				if (selected_task_properties.hasClass("collapsed_properties"))
					WF.Property.resizePropertiesPanel(selected_task_properties, 30); //30 is the height of the selected_task_properties when collapsed
				else
					setTimeout(function() { //must be with settimeout with at least .5 sec, bc the selected_task_properties has the css transition:.5s all;
						WF.Property.resizePropertiesPanel(selected_task_properties, selected_task_properties.outerHeight());
					}, 600);
		...
		loadConnectionPropertiesFromHtmlElm : function(properties_elm, connection_id) {
			...
			if (connection) {
				if (!WF.TaskFlow.connections_properties[connection_id]) {
					WF.TaskFlow.connections_properties[connection_id] = {};
				}
		
				this.setPropertiesFromHtmlElm(properties_elm, "connection_property_field", WF.TaskFlow.connections_properties[connection_id]);
			
				var source_id = connection.sourceId;
				var task_type = $("#" + source_id).attr("type");
			
				this.callTaskSettingsCallback(task_type, "on_load_connection_properties", [ properties_elm, connection, WF.TaskFlow.connections_properties[connection_id] ]);
			}
		...
		saveConnectionProperties : function(options) {
			...
			if (connection) {
				...
				status = this.callTaskSettingsCallback(task_type, "on_submit_connection_properties", [ selected_connection_properties, connection, WF.TaskFlow.connections_properties[connection_id] ]);
				...
				this.callTaskSettingsCallback(task_type, "on_complete_connection_properties", [ selected_connection_properties, connection, WF.TaskFlow.connections_properties[connection_id], status ]);
			}
		...
		saveConnectionPropertiesFromHtmlElm : function(properties_elm, connection_id) {
			...
			parse_str(query_string, WF.TaskFlow.connections_properties[connection_id]);
	
			//console.log(WF.TaskFlow.tasks_properties[task_id]);
		},
		...
		cancelConnectionProperties : function() {
			...
			if (connection) {
				...
				status = this.callTaskSettingsCallback(task_type, "on_cancel_connection_properties", [ selected_connection_properties, connection, WF.TaskFlow.connections_properties[connection_id] ]);
			}
		...
		loadTaskPropertiesFromHtmlElm : function(properties_elm, task_id) {
			if (!WF.TaskFlow.tasks_properties[task_id])
				WF.TaskFlow.tasks_properties[task_id] = {};
			
			this.setPropertiesFromHtmlElm(properties_elm, "task_property_field", WF.TaskFlow.tasks_properties[task_id]);
		
			var task = WF.TaskFlow.getTaskById(task_id);
			var task_type = task.attr("type");
			
			this.callTaskSettingsCallback(task_type, "on_load_task_properties", [ properties_elm, task_id, WF.TaskFlow.tasks_properties[task_id] ]);
		...
		saveTaskProperties : function(options) {
			...
			if (status) {
				status = this.callTaskSettingsCallback(task_type, "on_submit_task_properties", [ selected_task_properties, task_id, WF.TaskFlow.tasks_properties[task_id] ]);
				...
				this.callTaskSettingsCallback(task_type, "on_complete_task_properties", [ selected_task_properties, task_id, WF.TaskFlow.tasks_properties[task_id], status ]);
			}
		...
		saveTaskPropertiesFromHtmlElm : function(properties_elm, task_id) {
			var exits = WF.TaskFlow.tasks_properties[task_id] ? WF.TaskFlow.tasks_properties[task_id]["exits"] : null;
			
			WF.TaskFlow.tasks_properties[task_id] = {};
			WF.TaskFlow.tasks_properties[task_id]["exits"] = exits;
			...
			try {
				parse_str(query_string, WF.TaskFlow.tasks_properties[task_id]);
			}
			catch(e) {
				//alert(e);
		...
		continueSavingTaskProperties : function(task_id, task_properties_html_element) {
			...
			var exits = WF.TaskFlow.tasks_properties[task_id] && WF.TaskFlow.tasks_properties[task_id].exits; //if user empties the diagram and the properties popup is open and user clicks the save button, it will give an error bc the WF.TaskFlow.tasks_properties[task_id] was deleted when the user emptied the diagram. So we must do this check, to avoid the javascript error.
			if (exits) {
		...
		saveTaskPropertiesExits : function(task_id, task_properties_html_element) {
			...
			var exits = WF.TaskFlow.tasks_properties[task_id].exits;
			if (exits) {
				for (var exit_id in exits) {
			...
			if (exit_ids_to_delete.length > 0) {
				for (var i = 0; i < exit_ids_to_delete.length; i++) {
					...
					var j_exit_selector = j_task.find(" > ." + WF.TaskFlow.task_eps_class_name + " ." + WF.TaskFlow.task_ep_class_name + "[connection_exit_id=\"" + exit_id + "\"]");
			...
			for (var i in exits_to_add) {
				...
				WF.TaskFlow.addExtraSourceElementToTask(task_id, exit);
			}
				...
				if (connections) {
					...
					for (var i = 0; i < connections.length; i++) {
						...
						if (connection) {
							//preparing color
							if (exit["color"])
								WF.TaskFlow.changeConnectionColor(connection, exit["color"]);
							...
							if (exit["type"]) {
								...
								WF.TaskFlow.changeConnectionConnectorType(connection, exit["type"]);
							}
							else if (exit["overlay"]) 
								WF.TaskFlow.changeConnectionOverlayType(connection, exit["overlay"]);
						}
					}
				}
				
				var j_exit_selector = j_task.find(" > ." + WF.TaskFlow.task_eps_class_name + " ." + WF.TaskFlow.task_ep_class_name + "[connection_exit_id=\"" + exit_id + "\"]").first();
			...
			for (var i = 0; i < length; i++) {
				...
				if (exit_id && new_task_properties_exits.hasOwnProperty(exit_id)) {
					...
					//SORTING EPs IN TASK
					var ep = j_task.find(" > ." + WF.TaskFlow.task_eps_class_name + " ." + WF.TaskFlow.task_ep_class_name + "[connection_exit_id=\"" + exit_id + "\"]").first();
					if (i > 0) {
			...
			//console.log(WF.TaskFlow.tasks_properties[task_id].exits);
			WF.TaskFlow.tasks_properties[task_id].exits = sorted_task_properties_exits;
			//console.log(WF.TaskFlow.tasks_properties[task_id].exits);
		...
		cancelTaskProperties : function() {
			...
			var status = this.callTaskSettingsCallback(task_type, "on_cancel_task_properties", [ selected_task_properties, task_id, WF.TaskFlow.tasks_properties[task_id] ]);
	...
	WF.ContextMenu = { 
		...
		on_escape_key_press_handler : function(e) {
			if (e.which === 27 || e.keyCode === 27) //if escape key
				$("#" + WF.ContextMenu.main_tasks_menu_obj_id + " .task_menu.ui-draggable-dragging").data("escape_key_pressed", true).trigger('mouseup');
		},
		
		/* START: INIT METHODS */
		init : function() {
			var hash = Math.abs(WF.TaskFlow.main_tasks_flow_obj_id.hashCode());
			...
			$("#" + this.main_tasks_menu_obj_id).addClass("taskflowchartmenu");
			...
			tasks_menu_hide.draggable({
				...
				start: function(event, ui) {
					if (!tasks_menu_hide.hasClass("tasks_menu_hidden")) {
						...
						if (parent.hasClass("fixed_side_properties")) {
							selected_task_properties = $("#" + WF.Property.selected_task_properties_id);
							selected_connection_properties = $("#" + WF.Property.selected_connection_properties_id);
				...
				drag : function(event, ui) {
					//Note that any change here must be replicated in the WF.Property.updateFixedSidePropertiesPanelWidth method
					if (parent.hasClass("reverse")) {
		...
		initMenuTasks : function() {
			...
			var main_tasks_menu_obj = $("#" + WF.ContextMenu.main_tasks_menu_obj_id);
			var main_tasks_flow_obj = $("#" + WF.TaskFlow.main_tasks_flow_obj_id);
			var is_js_plumb = ExternalLibCloneBridge.isJsPlumb();
			
			//bind escape key event
			var doc = main_tasks_menu_obj[0].ownerDocument || document;
			$(doc).bind("keyup", WF.ContextMenu.on_escape_key_press_handler);
			
			//set draggable event to menu tasks
			for (var i = 0; i < elms.length; i++) {
				...
				var draggable_options = {
					...
					stop: function(e, obj) {
						...
						var drag_cancelled = j_elm.data("escape_key_pressed");
						j_elm.data("escape_key_pressed", null);
						...
						if (j_elm.data("start_dragging") == 1) {
							...
							if (!drag_cancelled) {
								if (!WF.ContextMenu.checkIfTaskMenuCanBeCreated( task_type, elm, to))
									WF.StatusMessage.showError("You cannot drop this Task on this position! Please try again...");
								else {
									var droppable = j_elm.data("droppable");
									...
									var is_first_task = WF.taskFlowChartObjOptions && WF.taskFlowChartObjOptions.hasOwnProperty("add_default_start_task") && WF.taskFlowChartObjOptions.add_default_start_task && (!droppable || $(droppable).is(main_tasks_flow_obj)) && main_tasks_flow_obj.find("." + WF.TaskFlow.task_class_name).length == 0;
									
									task_id = WF.ContextMenu.addTaskByType(task_type, obj_offset, droppable);
									var task = WF.TaskFlow.getTaskById(task_id);
									...
									if (task_id && is_first_task) {
										...
										task.addClass(WF.TaskFlow.start_task_class_name);
									}
								}
							}
							...
							},
							{
								...
								complete: function() {
									...
									if (task_id) {
										//Preparing task connections if droppable is already an existent connection
										if (droppable_connection)
											WF.ContextMenu.setTaskBetweenConnection(task, droppable_connection);
										
										//Calling success function
										WF.Property.callTaskSettingsCallback(task_type, "on_success_task_cloning", [ task_id ]);
									}
				...
				if (is_js_plumb)
					ExternalLibClone.draggable(elms[i], draggable_options);
				else
					$(elms[i]).draggable(draggable_options);
		...
		initTasksGroups : function() {
			...
			for (var i = 0; i < tasks_groups.length; i++) {
				...
				tasks_group.click(function() {
					...
					var tasks_group_tasks_elm = parent.children("." + WF.ContextMenu.tasks_group_tasks_class_name).first();
					...
					/*var cloned = $("#" + WF.ContextMenu.main_tasks_menu_obj_id + " .cloned_task").hide();
					
					var items = $(WF.ContextMenu.task_menu_obj_selector);
					items.css("left", "0px");
					items.css("top", "0px");
		...
		initContextMenuSettings : function(menu_type, available_menus_settings) {
			if (WF.Property.tasks_settings) {
				for (var task_type in WF.Property.tasks_settings) {
					if (WF.Property.isTaskSubSettingTrue(task_type, menu_type, "show_context_menu", true)) {
						var show_context_menu = false;
					
						for (var i = 0; i < available_menus_settings.length; i++) {
							if (WF.Property.isTaskSubSettingTrue(task_type, menu_type, available_menus_settings[i], true)) {
								show_context_menu = true;
								break;
							}
						}
					
						if (!WF.Property.hasTaskSetting(task_type, menu_type)) {
							eval ("WF.Property.tasks_settings[task_type]." + menu_type + " = {};");
						}
					
						eval ("WF.Property.tasks_settings[task_type]." + menu_type + ".show_context_menu = show_context_menu;");
					}
				}
			}
		},
		...
		initTaskContextMenu : function() {
			var html = '<ul id="' + this.task_context_menu_id + '" class="mycontextmenu ' + this.task_context_menu_classes + '">' + 
						'<li class="set_label"><a href="#" onClick="return ' + taskFlowChartObjVarName + '.ContextMenu.setSelectedTaskLabel();">Set Label</a></li>' + 
						'<li class="properties"><a href="#" onClick="return ' + taskFlowChartObjVarName + '.Property.showSelectedTaskProperties();">Properties</a></li>' + 
						'<li class="start_task"><a href="#" onClick="return ' + taskFlowChartObjVarName + '.ContextMenu.setSelectedStartTask();">Start Task</a></li>' + 
						'<li class="delete"><a href="#" onClick="return ' + taskFlowChartObjVarName + '.ContextMenu.deleteSelectedTask();">Delete</a></li>' + 
					'</ul>';
		
			$("#" + WF.TaskFlow.main_tasks_flow_obj_id).append(html);
			
			//this is already set in the WF.TaskFlow.addTargetElement method by: 'j_target_elm.addcontextmenu(;'
		...
		initConnectionContextMenu : function() {
			var html = '<ul id="' + this.connection_context_menu_id + '" class="mycontextmenu ' + this.connection_context_menu_classes + '">' +
						'<li class="set_label"><a href="#" onClick="return ' + taskFlowChartObjVarName + '.ContextMenu.setSelectedConnectionLabel();">Set Label</a></li>' +
						'<li class="properties"><a href="#" onClick="return ' + taskFlowChartObjVarName + '.Property.showSelectedConnectionProperties();">Properties</a></li>' +
						'<li class="connector_types"><a href="#">Connector Types</a>' + 
							'<ul>';
		
			var types = WF.TaskFlow.available_connection_connectors_type;
			for (var i = 0; i < types.length; i++) {
				var connector_type = types[i];
			
				html += 			'<li id="' + connector_type + '"><a href="#" onClick="return ' + taskFlowChartObjVarName + '.ContextMenu.setSelectedConnectionConnector(\'' + connector_type + '\');">' + connector_type + '</a></li>';
			}
			...
			var types = WF.TaskFlow.available_connection_overlays_type;
			for (var i = 0; i < types.length; i++) {
				var overlay_type = types[i];
			
				html += 			'<li id="' + overlay_type + '"><a href="#" onClick="return ' + taskFlowChartObjVarName + '.ContextMenu.setSelectedConnectionOverlay(\'' + overlay_type + '\');">' + overlay_type + '</a></li>';
			}
		
			html +=				'</ul>' + 
						'</li>' +
						'<li class="delete"><a href="#" onClick="return ' + taskFlowChartObjVarName + '.ContextMenu.deleteSelectedConnection();">Delete</a></li>' +
					'</ul>';
		
			$("#" + WF.TaskFlow.main_tasks_flow_obj_id).append(html);
			
			//For some reason ExternalLibClone needs this to be build, bc the the 'ExternalLibClone.bind("contextmenu", ...);' in the WF.TaskFlow.init method is not building the connections menu. So we need to hard code this part here!
		...
		initConnectionAddNewTaskPanel : function() {
			var html ='<div id="' + this.connection_add_new_task_panel_id + '" class="connection_add_new_task_panel myfancypopup">' + 
					...
					'	<select></select>' + 
				 	'	<div class="generic_buttons">' + 
						'	<input type="button" class="cancel" value="Cancel" onClick="' + taskFlowChartObjVarName + '.ContextMenu.cancelAddNewTaskToConnection(this)" />' + 
				 		'	<input type="button" class="save" value="Add" onClick="' + taskFlowChartObjVarName + '.ContextMenu.addNewTaskToConnection(this)" />' + 
					'	</div>' + 
				 	'</div>';
		...
		destroy : function() {
			//unbind escape key event
			var main_tasks_menu_obj = $("#" + this.main_tasks_menu_obj_id);
			var doc = main_tasks_menu_obj[0].ownerDocument || document;
			$(doc).unbind("keyup", this.on_escape_key_press_handler);
			...
			if (this.isContextMenuActive()) {
				...
				if (pos != -1)
					MyContextMenu.setBuildContextMenuIds( jQuery.grep(MyContextMenu.getBuildContextMenuIds(), function(id, idx) {
						return id != WF.ContextMenu.task_context_menu_id;
					}) );
				...
				if (pos != -1)
					MyContextMenu.setBuildContextMenuIds( jQuery.grep(MyContextMenu.getBuildContextMenuIds(), function(id, idx) {
						return id != WF.ContextMenu.connection_context_menu_id;
					}) );
			}
		},
		...
		prepareTaskMenu : function(originalEvent, task) {
			...
			if (task_type) {
				...
				for (var i = 0; i < menus.length; i++)
					if (WF.Property.isTaskSubSettingTrue(task_type, "task_menu", "show_" + menus[i] + "_menu", true))
			...
			//CALL TASK CALLBACK
			WF.Property.callTaskSettingsCallback(task_type, "on_show_task_menu", [ task_id, j_task, task_context_menu ]);
		},
		...
		prepareConnectionMenu : function(originalEvent, connection) {
			...
			if (task_type) {
				...
				for (var i = 0; i < menus.length; i++)
					if (WF.Property.isTaskSubSettingTrue(task_type, "connection_menu", "show_" + menus[i] + "_menu", true)) 
			...
			WF.Property.callTaskSettingsCallback(task_type, "on_show_connection_menu", [ connection.id, connection, connection_context_menu ]);
		},
		...
		getContextMenuConnectionId : function() {
			...
			if (!id) {
				...
				if (obj) {
					id = WF.TaskFlow.getEventTargetConnectionId(obj);
			
					if (!id && event && event.target)
						id = WF.TaskFlow.getEventTargetConnectionId(event.target);
				}
			}
		...
		setSelectedConnectionLabel : function(options) {
			...
			return WF.TaskFlow.setConnectionLabelByConnectionId(connection_id);
		},
		...
		setSelectedConnectionConnector : function(selected_connector_type, options) {
			...
			if (selected_connector_type) {
				...
				if (connection && WF.TaskFlow.changeConnectionConnectorType(connection, selected_connector_type)) {
					//PREPARE CONNECTOR TYPES
					...
					if (current_zoom != -1)
						WF.TaskFlow.repaintConnection(connection);
		...
		setSelectedConnectionOverlay : function(selected_overlay_type, options) {
			...
			if (selected_overlay_type) {
				...
				if (connection && WF.TaskFlow.changeConnectionOverlayType(connection, selected_overlay_type)) {
					//PREPARE OVERLAY TYPES
		...
		addNewTaskToConnection : function(elm) {
			...
			if (connection && task_type) {
				if (!this.addTaskByTypeToConnection(task_type, connection))
					WF.StatusMessage.showError("Error: New task not added successfully! Please check your workflow and make the necessary changes manually...");
			}
			else if (!task_type)
				WF.StatusMessage.showError("Error: Wrong task to be added.");
			else
				WF.StatusMessage.showError("Error: Could not found connection where this new task should be added.");
		...
		getContextMenuTaskId : function() {
			...
			if (!id) {
				...
				if (obj) {
					var id = WF.TaskFlow.getEventTargetTaskId(obj);
			
					if (!id && event && event.target)
						id = WF.TaskFlow.getEventTargetTaskId(event.target);
				}
			}
		...
		setSelectedTaskLabel : function(options) {
			...
			return WF.TaskFlow.setTaskLabelByTaskId(task_id);
		},
		...
		setSelectedStartTask : function(options) {
			...
			$("#" + this.task_context_menu_id + " .start_task").each(function(i, elm) {
				if (j_task.attr("is_start_task") > 0) {
					...
					j_task.removeClass(WF.TaskFlow.start_task_class_name);//Calling callback on end selection
				}
				else {
					//only get items from correspondent scope
					var task_scope = WF.TaskFlow.getTaskParentTasks(j_task).first();
					task_scope = task_scope[0] ? task_scope : main_tasks_flow_obj;
					
					var items = task_scope.find("." + WF.TaskFlow.task_class_name + "[is_start_task]");
					var filtered_items = [];
					...
					for (var i = 0; i < items.length; i++) {
						var item = $(items[i]);
						var parents = WF.TaskFlow.getTaskParentTasks(item);
						var invalid = false;
						
						for (var j = 0; j < parents.length; j++) {
							var parent = $(parents[j]);
							
							if ( parent.find(" > ." + WF.TaskFlow.task_eps_class_name + " ." + WF.TaskFlow.task_ep_class_name).length > 0 && !parent.is(task_scope)) {
								invalid = true;
								break;
							}
						}
				...
				WF.Property.callTaskSettingsCallback(task_type, "on_complete_select_start_task", [ task_id, j_task ]);
			}).promise().done(function () {
				//Calling callback on end selection
				WF.Property.callTaskSettingsCallback(task_type, "on_complete_select_start_tasks", [ task_id, j_task ]);
			});
		},
		...
		deleteSelectedTask : function(options) {
			...
			return WF.TaskFlow.deleteTask(task_id);
		},
		...
		addTask : function(task_props) {
			...
			if (!task_type || !task_id) {
				WF.StatusMessage.showError("Error: Task Type or Id is undefined for task '" + task_type + "' with id '" + task_id + "' and label '" + task_label + "'.");
				return false;
			}
			
			task_type = task_type.toLowerCase();
			task_label = task_label ? task_label : WF.TaskFlow.default_label;
			...
			var task_menu = $("#" + WF.ContextMenu.main_tasks_menu_obj_id + " .task_" + task_type);
			...
			if (connection_exits_props && connection_exits_props.length > 0)
				exit_html = exit_html.replace(/#task_ep_class_name#/g, WF.TaskFlow.task_ep_class_name);
			else
				exit_html = "";
			
			//replace html entities
			var task_menu_label_encoded = ("" + task_menu_label).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
			var task_label_encoded = ("" + task_label).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
			var task_title_encoded = ("" + task_title).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
			
			//Creating task html
			var task_html = this.task_html;
			task_html = task_html.replace(/#task_class_name#/g, WF.TaskFlow.task_class_name);
			task_html = task_html.replace(/#task_info_class_name#/g, WF.TaskFlow.task_info_class_name);
			task_html = task_html.replace(/#task_info_tag_class_name#/g, WF.TaskFlow.task_info_tag_class_name);
			task_html = task_html.replace(/#task_info_label_class_name#/g, WF.TaskFlow.task_info_label_class_name);
			task_html = task_html.replace(/#task_label_class_name#/g, WF.TaskFlow.task_label_class_name);
			task_html = task_html.replace(/#task_full_label_class_name#/g, WF.TaskFlow.task_full_label_class_name);
			task_html = task_html.replace(/#task_eps_class_name#/g, WF.TaskFlow.task_eps_class_name);
			task_html = task_html.replace(/#task_short_actions_class_name#/g, WF.TaskFlow.task_short_actions_class_name);
			...
			task_html = task_html.replace(/#task_info_label#/g, task_menu_label_encoded);
			task_html = task_html.replace(/#task_label#/g, task_label_encoded);
			...
			task_html = task_html.replace(/#task_title#/g, task_title_encoded);
			task_html = task_html.replace(/#start_task_class_name#/g, is_start_task ? WF.TaskFlow.start_task_class_name : "");
			...
			var short_actions = task.find("." + WF.TaskFlow.task_short_actions_class_name);
	  		...
	  		if (show_context_menu)
				context_menu_action.click(function(originalEvent) {
					if (originalEvent) {
		  			...
		  			//show task menu
		  			WF.ContextMenu.showTaskMenu(originalEvent, task[0]);
				});
			else
				context_menu_action.remove();
			
			//Calling success function
			WF.Property.callTaskSettingsCallback(task_type, "on_success_task_append", [ task_id ]);
			...
			var status = WF.TaskFlow.addTargetElement(task[0], cep) == task_id;
			...
			//resize parent task with the new width and height according with this inner task
			WF.TaskFlow.resizeTaskParentTask(task);
			
			if (status) {
				//add source endpoints
				var source_element = task.find(WF.TaskFlow.source_selector);
				if (source_element[0])
					WF.TaskFlow.addSourceElement(source_element[0], task[0], cep);
				
				if (connection_exits_props && connection_exits_props.length > 1)
					for (var i = 1; i < connection_exits_props.length; i++)
						WF.TaskFlow.addExtraSourceElementToTask(task_id, connection_exits_props[i]);
		...
		loadTask : function(task_props) {
			var task_type = task_props["task_type"];
			var task_id = task_props["task_id"];
			task_props["is_resizable_task"] = WF.Property.isTaskSettingTrue(task_type, "is_resizable_task", false);
			//console.log("loadTask:"+task_props["task_tag"]+":"+task_props["is_resizable_task"]);
			
			var status = this.addTask(task_props) == task_id;
			
			if (status) {
				WF.Property.callTaskSettingsCallback(task_type, "on_success_task_creation", [ task_id ]);
		...
		addTaskByType : function(task_type, offset, droppable) {
			...
			if (!task_id) {
				WF.StatusMessage.showError("There was an error trying to generating a new task id. Please try again later...");
				return false;
			}
			...
			var is_resizable_task = WF.Property.isTaskSettingTrue(task_type, "is_resizable_task", false);
			...
			if (status) {
				var properties = $("#" + WF.TaskFlow.main_tasks_properties_obj_id + " .task_properties_" +  task_type.toLowerCase()).html();
				...
				status = WF.Property.loadTaskPropertiesFromHtmlElm( j_div_tmp, task_id);
				if (status)
					WF.Property.saveTaskPropertiesFromHtmlElm( j_div_tmp, task_id);
				j_div_tmp.html("");
				
				WF.Property.callTaskSettingsCallback(task_type, "on_success_task_creation", [ task_id ]);
			}
			else {	
				WF.StatusMessage.showError("Error trying to create task '" + task_type + "' with id '" + task_id + "'.");
				return false;
			}
		...
		addTaskByTypeToConnection : function(task_type, connection) {
			...
			if (!connection) {
				WF.StatusMessage.showError("Error: no connection seleted.");
				return false;
			}
			else if (!task_type) {
				WF.StatusMessage.showError("Error: task type undefined.");
				return false;
			}
			...
			if (new_task_id) {
				var j_task = WF.TaskFlow.getTaskById(new_task_id);
		...
		setTaskBetweenConnection : function(task, connection) {
			...
			if (WF.TaskFlow.deleteConnection(connection.id, true))
				connection_status = WF.TaskFlow.connect(connection.sourceId, task_id, connection_label, connection_exit_type, connection_exit_overlay, {
					id : connection_exit_id,
					color : connection_exit_color,
				});
			...
			for (var exit_id in task_exits) {
				...
				var con_status = WF.TaskFlow.connect(task_id, connection.targetId, exit_label, connection_exit_type, connection_exit_overlay, {
					id : exit_id,
			...
			if (connection_status)
				WF.Property.callTaskSettingsCallback(task_type, "on_success_task_between_connection", [ task_id ]);
		...
		prepareTaskDroppables : function(task, options) {
			//Setting inner droppables if exists
			var inner_droppables = task.find("." + WF.TaskFlow.task_droppable_class_name);
			...
			$.each(inner_droppables, function(idx, inner_droppable) {
				...
				if (inner_droppable.css("position") != "absolute") {
					...
					WF.StatusMessage.showError("This task contains droppable elements without the position absolute. Droppable elements must be absolute! Error in task: '" + task_tag + "' with id '" + task_id + "'.");
				}
			});
		},
		...
		generateNewTaskId : function(task_type) {
			...
			do {	
				task_id = 'task_' + task_type + '_' + Math.floor(Math.random()*1100) + '_' + Math.abs(WF.TaskFlow.main_tasks_flow_obj_id.hashCode());
			
				if (!WF.TaskFlow.TaskIdExists(task_id)) {
					return task_id;
				}
		...
		checkIfTaskCanBeCreated : function(task_type, elm, task_offset) {
			return WF.Container.canTaskBeCreatedInsideOfSelectedContainer(task_type, elm, task_offset);
		},
	
		checkIfTaskMenuCanBeCreated : function(task_type, elm, task_offset) {
			return WF.Container.canTaskBeCreatedInsideOfSelectedContainer(task_type, elm, task_offset) && WF.Container.canTaskBeCreatedInsideOfMainTaskFlowContainer(task_type, elm, task_offset);
		},
		...
		sortTasksMenu : function() {
			...
			//remove scroll from tasks_menu
			var has_scroll = tasks_menu.hasClass("scroll");
			
			if (has_scroll)
				tasks_menu.removeClass("scroll");
			...
			//add scroll to tasks_menu
			if (has_scroll)
				tasks_menu.addClass("scroll");
		},
		...
		flipPanelsSide : function() {
			...
			var properties_panels = $("#" + WF.Property.selected_task_properties_id + ",#" + WF.Property.selected_connection_properties_id);
	...
	WF.Container = { 
		...
		initContainersSettings : function() {
			var containers = $("#" + WF.TaskFlow.main_tasks_flow_obj_id + " ." + this.tasks_containers_class_name);
		...
		canTaskBeCreatedInsideOfSelectedContainer : function(task_type, elm, task_offset) {
			...
			var task_top = task_offset.top;
			var task_left = task_offset.left;
			
			task_width = task_width * current_zoom;
			task_height = task_height * current_zoom;
			//console.log(task_top+":"+task_height);
			//console.log(task_left+":"+task_width);
			...
			if (!$.isEmptyObject(this.tasks_containers)) {
				...
				if (containers_id.length > 0) {
					for (var i = 0; i < containers_id.length; i++) {
						...
						var container = $("#" + WF.TaskFlow.main_tasks_flow_obj_id + " #" + container_id)[0];
						...
						if (container) {
							...
							container_width = container_width * current_zoom;
							container_height = container_height * current_zoom;
							//console.log(container_top+":"+container_height);
							//console.log(container_left+":"+container_width);
							...
							if (task_left >= container_left && task_top >= container_top) {
								is_inside = true;
								
								if (!allow_automatic_width_resize && task_left + task_width > container_left + container_width)
									is_inside = false;
								
								if (!allow_automatic_height_resize && task_top + task_height > container_top + container_height)
									is_inside = false;
							}
		...
		changeContainerSize : function(container_id, new_width, new_height) {
			var container = $("#" + WF.TaskFlow.main_tasks_flow_obj_id + " #" + container_id);
			...
			if (container && container.id) {
				...
				if (s.allow_automatic_width_resize && new_width < width) {
					new_width = width;
					WF.StatusMessage.showMessage("You cannot decrease the width for the container:'" + container_id + "'!");
				}
			
				if (s.allow_automatic_height_resize && new_height < height) {
					new_height = height;
					WF.StatusMessage.showMessage("You cannot decrease the height for this container:'" + container_id + "'!");
				}
			...
			if (container && container.id && tasks && tasks.length > 0) {
				...
				for (var i = 0; i < tasks.length; i++) {
					...
					if (new_task_left != task_offset.left || new_task_top != task_offset.top) {
						...
						WF.TaskFlow.repaintTask(j_task);
					}
				}
			}
		},
	...
	WF.TaskFile = { 
		...
		init : function() {
			this.update_task_panel_id += "_" + Math.abs(WF.TaskFlow.main_tasks_flow_obj_id.hashCode());
			this.update_connection_panel_id += "_" + Math.abs(WF.TaskFlow.main_tasks_flow_obj_id.hashCode());
			
			this.read();
			
			if (this.auto_save_interval > 0)
				this.auto_save_set_interval_func_id = setInterval(function() {
					WF.TaskFile.autoSave();
			...
			if (get_tasks_file_url) {
				...
				var ajax_options = {
					...
					success : function(data, text_status, jqXHR) {
						WF.TaskFile.parseTasksFileData(data, text_status, jqXHR);
						...
						WF.TaskFile.startAutoSave({reset_saved_data_obj : true});
					},
					error : function(jqXHR, textStatus, errorThrown) { 
						var msg = jqXHR.responseText ? "\n" + jqXHR.responseText : "";
						WF.StatusMessage.showError("Error: Workflow not loaded!\nPlease try again..." + msg);
						
						if (options && typeof options["error"] == "function")
							options["error"](jqXHR, textStatus, errorThrown);
						
						//stop auto save function
						WF.TaskFile.stopAutoSave();
					},
				};
		...
		parseTasksFileData : function(data, text_status, jqXHR) {
			WF.TaskFile.updateFileSettings(data); //save diagram settings
			
			var containers = data && data.containers ? data.containers : null;
			WF.TaskFile.updateContainers(containers);
			
			var tasks = data && data.tasks ? data.tasks : null;
			var loaded_data = WF.TaskFile.prepareLoadedTasks(tasks);
			WF.TaskFile.updateTasks(loaded_data);
			
			if (WF.TaskFile.on_success_read && typeof WF.TaskFile.on_success_read == "function")
				WF.TaskFile.on_success_read(data, text_status, jqXHR);
			
			//we need to resize the containers again bc the updateTasks may increase the tasks_flow panel again...
			WF.Container.automaticIncreaseContainersSize();
		},
		...
		getWorkFlowData : function() {
			...
			var elms = main_tasks_flow_obj.find("." + WF.Container.tasks_containers_class_name);	
			...
			for (var i = 0; i < elms.length; i++) {
				...
				containers[container_id] = {
					...
					left: parseInt(position.left + sl), 
					top: parseInt(position.top + st)
				};
			}
			//console.log(containers);
			
			elms = $(WF.TaskFlow.target_selector);
			
			for (var i = 0; i < elms.length; i++) {
				var task = $(elms[i]);
				var is_inner_task = false;
				
				if (!task.parent().is(main_tasks_flow_obj) && WF.TaskFlow.getTaskParentTasks(task)[0])
					is_inner_task = true;
					
				var task_id = task.attr("id");
				tasks[task_id] = {};
				
				var info = task.find(" > ." + WF.TaskFlow.task_label_class_name + " span")[0];
			...
			for (var i = 0; i < elms.length; i++) {
				...
				if (!task_parent.is(main_tasks_flow_obj)) {
					var task_parent_task = WF.TaskFlow.getTaskParentTasks(task).first();
			...
			for (var i = 0; i < connections.length; i++) {
				...
				var color = connection.getPaintStyle().strokeStyle;
				var properties = WF.TaskFlow.connections_properties[connection_id];
			
				if (!type)
					type = connection.connector.type;
			
				if (!overlay)
					overlay = WF.TaskFlow.default_connection_overlay;
				
				label = label ? label.getLabel() : WF.TaskFlow.default_label;
				label = label == "" || label == "..." ? WF.TaskFlow.default_label : label;
			...
			if ($.isPlainObject(WF.TaskFile.file_other_data))
				for (var key in WF.TaskFile.file_other_data)
					if (key != "containers" && key != "tasks" && key != "settings")
						workflow_data[key] = WF.TaskFile.file_other_data[key];
			
			workflow_data["settings"] = WF.TaskFile.file_settings;
			...
			if (set_tasks_file_url) {
				...
				$.ajax({
					...
					success : function(data, textStatus, jqXHR) {
						if (data.trim() == "1") {
							status = true;
							!silent && WF.StatusMessage.showMessage("Workflow saved successfully!");
						}
						else if (data.trim() == "2") {
							WF.TaskFile.saved_data_obj = old_saved_data_obj; //if error, sets the old obj so it tries again the next time... This is used in the autoSave function.
							...
							if (confirm("The system detected that this workflow was changed by someone else and the current workflow that you are seeing is not the most updated.\nPlease choose one of the following options:\n- Click continue to save your changes and overwrite the other people's changes.\n- Or click cancel and then refresh this workflow, make your new changes and then click save again."))
								status = WF.TaskFile.save(set_tasks_file_url, options);
						}
						else {
							WF.TaskFile.saved_data_obj = old_saved_data_obj; //if error, sets the old obj so it tries again the next time... This is used in the autoSave function.
							
							(!silent || do_not_silent_errors) && WF.StatusMessage.showError("Error: Workflow saved unsuccessfully!\nPlease try again..." + (data ? "\n" + data : ""));
						}
						
						if (WF.TaskFile.on_success_save && typeof WF.TaskFile.on_success_save == "function")
							WF.TaskFile.on_success_save(data, textStatus, jqXHR);
			...
			if (get_tasks_file_url) {
				...
				if (status) {
					...
					if (!div) {
						...
						$("#" + WF.TaskFlow.main_tasks_flow_obj_id).append(div);
						...
						var div_html = '<div class="title">Please select the tasks that you wish to update:</div>'
							...
							+ '<div class="content"></div>'
							+ '<div class="buttons">'
								+ '<input type="button" class="continue" value="Continue" onClick="$(this).attr(\'disabled\', \'disabled\');return ' + taskFlowChartObjVarName + '.TaskFile.startUpdate();" />'
								+ '<input type="button" class="cancel" value="Cancel" onClick="$(this).attr(\'disabled\', \'disabled\');return ' + taskFlowChartObjVarName + '.TaskFile.cancelUpdate();" />'
							+ '</div>';
					...
					$.ajax({
						...
						dataType : "json",
						success : function(data, text_status, jqXHR) {
							WF.TaskFile.update_data = data;
							WF.TaskFile.selected_task_ids = null;
							
							var tasks = data && data.tasks ? data.tasks : null;
							if (tasks) {
								var existent_tasks = new Array();
						
								var elms = $(WF.TaskFlow.target_selector);
								for (var i = 0; i < elms.length; i++) {
								...
								WF.StatusMessage.showMessage("No DB tables available...");
								MyFancyPopupClone.hidePopup();
							}
						},
						error : function(jqXHR, textStatus, errorThrown) { 
		...
		startUpdate : function() {
			...
			if (!div) {
				...
				var div_html = '<div class="title">Please select the connections that you wish to keep:</div>'
					...
					+ '</div>'
					+ '<div class="buttons">'
						+ '<input type="button" class="continue" value="Continue" onClick="$(this).attr(\'disabled\', \'disabled\');return ' + taskFlowChartObjVarName + '.TaskFile.continueUpdate();" />'
						+ '<input type="button" class="cancel" value="Cancel" onClick="$(this).attr(\'disabled\', \'disabled\');return ' + taskFlowChartObjVarName + '.TaskFile.cancelUpdate();" />'
					+ '</div>';
				...
				for (var task_id in tasks) {
					...
					if (task.id) {
						if ($.inArray(task.id, selected_task_ids) != -1) {
							var connections = WF.TaskFlow.getSourceConnections(task.id);	
							...
							connections = WF.TaskFlow.getTargetConnections(task.id);	
							for (var i in connections) {
								var connection = connections[i];
								...
								if (!source_task) {
									source_task = WF.TaskFlow.tasks_properties[connection.sourceId];
									
									if (source_task && !task_labels[ connection.sourceId ])
										task_labels[ connection.sourceId ] = WF.TaskFlow.getTaskLabelByTaskId(connection.sourceId);
								}
							...
							if (task.exits)
								for (var exit_id in task.exits) {
									...
									for (var i = 0; i < exits.length; i++) {
										...
										if ($.inArray(exit.task_id, selected_task_ids) != -1 || WF.TaskFlow.getTaskById(exit.task_id).length > 0) { //If task has a connection with another selected task or with an existent task in the workflow.
						...
						else if (task.exits && WF.TaskFlow.getTaskById(task.id).length > 0) //if task is not selected but exists in workflow and has a connection to a selected task.
							for (var exit_id in task.exits) {
								var exits = task.exits[exit_id];
								exits = $.isArray(exits) ? exits : [exits];
			...
			if (loaded_tasks && selected_task_ids) {
				...
				for (var i = 0; i < loaded_tasks.length; i++) {
					...
					if ($.inArray(task.id, selected_task_ids) != -1) {
						...
						var connections = WF.TaskFlow.getTargetConnections(task.id);
						for (var j in connections) {
							var connection = connections[j];
							
							if ($.inArray(task.id, selected_existent_connections[connection.sourceId]) != -1) {
								var exit_id = connection.getParameter("connection_exit_id");
								var task_properties = WF.TaskFlow.tasks_properties[connection.sourceId];
						...
						for (var j = 0; j < connections_props.length; j++) {
							...
							var overlay = connection.getParameter("connection_exit_overlay");
							var color = connection.getPaintStyle().strokeStyle;
							var properties = WF.TaskFlow.connections_properties[connection.id];
				...
				for (var i = 0; i < loaded_connections.length; i++) {
					...
					if ($.inArray(task.id, selected_task_ids) != -1 || WF.TaskFlow.getTaskById(task.id).length > 0) {
				...
				$("#" + this.update_connection_panel_id).fadeOut("fast", function() {
					...
					WF.TaskFile.updateTasks(new_loaded_data);
					
					if (WF.TaskFile.on_success_update && typeof WF.TaskFile.on_success_update == "function") {
						WF.TaskFile.on_success_update(this.update_data);
		...
		updateContainers : function(containers) {
			if (containers) {
				...
				WF.Container.initContainersSettings();
				WF.Container.automaticIncreaseContainersSize();
			}
		},
		...
		updateTasks : function(loaded_data) {
			...
			if (loaded_tasks) {
				ExternalLibCloneBridge.setRepaintStatus(false);
				...
				for (var i = 0; i < loaded_tasks.length; i++) {
					...
					WF.TaskFlow.tasks_properties[task.id] = task.properties;
					var task_props = {
						task_type: task.type, 
						task_tag: task.tag, 
						task_id: task.id, 
					...
					if (WF.ContextMenu.loadTask(task_props) != task.id)
						WF.StatusMessage.showError("Error trying to create task " + task.type + " with id '" + task.id + "' and label '" + task.label + "'.");
				}
				...
				for (var i = 0; i < loaded_connections.length; i++) {
					...
					var connection = WF.TaskFlow.connect(task.id, exit.task_id, exit_label, exit.type, exit.overlay, connection_exit_props);
			
					if (exit.properties && connection.id)
						WF.TaskFlow.connections_properties[connection.id] = exit.properties;
				}
				...
				for (var i = 0; i < loaded_tasks.length; i++) {
					...
					if (task.hasOwnProperty("tasks") && task.tasks) {
						...
						var task_elm = WF.TaskFlow.getTaskById(task.id);
						
						if (task_elm[0]) {
							var droppable = null;
							var is_resizable_task = WF.Property.isTaskSettingTrue(task.type, "is_resizable_task", false);
							
							for (var inner_task_id in inner_tasks) {
								var path = inner_tasks[inner_task_id];
								var inner_task_elm = WF.TaskFlow.getTaskById(inner_task_id);
								
								if (!inner_task_elm[0]) {
									inner_task_id = WF.TaskFlow.getRealTaskIdFromCaseInsensitiveId(inner_task_id);
									inner_task_elm = WF.TaskFlow.getTaskById(inner_task_id);
								}
							...
							//Do not add the 'WF.TaskFlow.resizeTaskParentTask(droppable)' bc since we are loading the width and height of the task, we don't need this anymore, and there could be a situation where the user really wants a specific size and if we call resizeTaskParentTask the size will be overwrited.
							//resize droppable task and its' parents with the new width and height according with the new dropped inner task
	...
	WF.StatusMessage = { 
		...
		init : function() {
			this.message_html_obj_id += "_" + Math.abs(WF.TaskFlow.main_tasks_flow_obj_id.hashCode());
			this.message_html_obj = $('<div id="' + this.message_html_obj_id + '" class="workflow_message"></div>');
			
			$("#" + WF.TaskFlow.main_tasks_flow_obj_id).after(this.message_html_obj);
		...
		showMessage : function(message, message_class) {
			...
			try { //if message contains a full html page with head and body we will get a javascript error. So we need to catch it.
				...
				message_html_obj = $("#" + WF.TaskFlow.main_tasks_flow_obj_id).children('#' + this.message_html_obj_id); //sometimes the this.message_html_obj looses the reference for the object
		...
		getMessageElement : function(message, message_class) {
			var message_html_obj = this.getMessageHtmlObj();
			var width = $("#" + WF.TaskFlow.main_tasks_flow_obj_id).width() - 15;
		...
		prepareMessage : function(status_message, timeout) {
			...
			var z_index = WF.TaskFlow.getTasksFlowElementsMaxZIndex();
			...
			close_icon.click(function(event) {
				...
				WF.StatusMessage.removeMessage(this);
				WF.StatusMessage.getMessageHtmlObj().css("z-index", czi);
